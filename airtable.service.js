(function (global) {
  ...
  class AirtableService {
    ...
    // ---- URLs for main table ----
    listUrl(offset) {
      const base = `https://api.airtable.com/v0/${encodeURIComponent(this.baseId)}/${encodeURIComponent(this.tableId)}?view=${encodeURIComponent(this.viewId)}`;
      const url = offset ? `${base}&offset=${encodeURIComponent(offset)}` : base;
      AIRTABLE_LOGGER.debug("listUrl", url);
      return url;
    }
    tableUrl(id) {
      const base = `https://api.airtable.com/v0/${encodeURIComponent(this.baseId)}/${encodeURIComponent(this.tableId)}`;
      const url = id ? `${base}/${encodeURIComponent(id)}` : base;
      AIRTABLE_LOGGER.debug("tableUrl", url);
      return url;
    }

    // ---- URLs for arbitrary source tables ----
    otherListUrl(tableId, viewId, offset) {
      const base = `https://api.airtable.com/v0/${encodeURIComponent(this.baseId)}/${encodeURIComponent(tableId)}?view=${encodeURIComponent(viewId)}`;
      return offset ? `${base}&offset=${encodeURIComponent(offset)}` : base;
    }
    otherTableUrl(tableId, id) {
      const base = `https://api.airtable.com/v0/${encodeURIComponent(this.baseId)}/${encodeURIComponent(tableId)}`;
    ...
    async fetchAllRecords(signal) {
      ...
      while (url) {
        page++;
        AIRTABLE_LOGGER.time(`page ${page}`);
        ...
        AIRTABLE_LOGGER.timeEnd(`page ${page}`);
        AIRTABLE_LOGGER.info("list",`page ${page} records`, len);
      }
      AIRTABLE_LOGGER.info("fetchAllRecords","total", out.length);
